...
const DetailProductScreen = () => {
  ...
  const [formEdit, setFormEdit] = useState<Product>({
    id: "",
    author: "", // Cambiado de code a banda
    book: "", // Cambiado de nameProduct a album
    price: 0,
    stock: 0,
    description: "",
  });
  ...
  const handleUpdateProducts = async () => {
    const dbRef = ref(
      dbRealTime,
      "libros/" + auth.currentUser?.uid + "/" + formEdit.id
    );

    try {
      // Actualizar el producto en Firebase
      await update(dbRef, {
        author: formEdit.author,
        book: formEdit.book,
  ...
  return (
    <View style={styles.rootDetail}>
      <View>
        <Text variant="headlineSmall">Titulo: {formEdit.book} </Text>
        <TextInput
          label="Libro"
          value={formEdit.book}
          onChangeText={(value) => handleSetValues("book", value)} // Cambiado de code a banda
        />
        <Divider />
      </View>
      <View>
        <Text variant="bodyLarge">Autor: {formEdit.author} </Text>
        <TextInput
          label="Autor"
          value={formEdit.author}
          onChangeText={(value) => handleSetValues("author", value)} // Cambiado de nameProduct a album
        />
        <Divider />
      </View>
